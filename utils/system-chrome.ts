...
export const CHROMEDRIVER   = '/opt/homebrew/bin/chromedriver';
...
class WdClient {
  ...
  private async json(method: string, p: string, body?: unknown): Promise<unknown> {
  ...
  async newSession(caps: object): Promise<void> {
    const data = await this.json('POST', '/session', { capabilities: { alwaysMatch: caps } }) as {
      value: { sessionId?: string; capabilities?: { 'goog:chromeOptions'?: { debuggerAddress?: string } } }
    };
